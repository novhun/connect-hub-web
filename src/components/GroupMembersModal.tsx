...
import { X, Search, Users, ShieldCheck, Loader2, MessageSquare, UserCheck } from 'lucide-react';
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-3 divide-y divide-gray-50">
          {loading ? (
            <div className="py-12 flex flex-col items-center justify-center text-gray-400 gap-2">
              ...
              <span className="text-xs">{language === 'km' ? 'កំពុងផ្ទុកបញ្ជីសមាជិក...' : 'Loading members...'}</span>
            </div>
          ) : filteredMembers.length === 0 ? (
            <div className="py-12 text-center text-gray-400 space-y-2">
              <Users className="w-8 h-8 mx-auto text-gray-300" />
              <p className="text-xs font-medium">{t('groups.noMembersYet')}</p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <span className="text-[10px] text-gray-600 bg-gray-100 font-medium px-2 py-0.5 rounded-full">
        ...
        <div className="p-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500 font-medium">
          <span>{filteredMembers.length} {t('groups.allMembers')}</span>
          <button
            onClick={onClose}
